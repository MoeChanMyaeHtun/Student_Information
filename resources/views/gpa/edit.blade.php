...
<div class="container-fluid p-4">
    ...
    <form action="{{ route('gpas.create.validate') }}" method="POST">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                <label for="semester" class="form-label">Current Semester</label><span class="text-danger">*</span>
                <select name="semester" class="form-select @error('semester') is-invalid @enderror">
                    ...
                    <option value="{{ $semester->value }}"
                        {{ old('semester', $gpa->semester) == $semester->value ? 'selected' : '' }}>
                        {{ $semester->value }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6">
                ...
                <select name="student_id" class="form-control @error('student_id') is-invalid @enderror">
                    ...
                    <option value="{{ $student->student_id }}"
                        {{ old('student_id', $gpa->student_id) == $student->student_id ? 'selected' : '' }}>
...
<div class="modal fade show" id="confirmModal" style="display: block;" tabindex="-1" aria-modal="true" role="dialog">
    <div class="modal-dialog modal-lg">
        <form method="POST" action="{{ route('gpas.update',$gpa->gpa_id) }}" enctype="multipart/form-data">
            @csrf
            @method("PUT")
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <a href="{{ route('gpas.edit',$gpa->gpa_id) }}" class="btn btn-secondary">Cancel</a>
                    <button type="submit" class="btn btn-primary">Update</button>
                </div>
            </div>
        </form>
